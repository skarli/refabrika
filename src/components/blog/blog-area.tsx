...
// Fallback data for when Sanity has no content
const fallbackBlogs = blogDataOne.slice(0, 3);

export default function BlogArea({
  sectionTitle = "Learn our recent journal",
  buttonText = "Learn all news",
  blogs = [],
}: BlogAreaProps) {
              ...
              {hasSanityBlogs
                ? blogs.map((blog, idx) => (
                    <Link href={`/blog/${blog.slug?.current}`} key={blog._id || idx}>
                      <article className="blog fade-anim" data-delay={`0.${(idx + 1) * 15}s`}>
                        <div className="thumb">
                          <Image
                            src={getImageUrl(blog.thumbnail, 410, 290)}
                            alt={blog.title || "Blog"}
                            width={410}
                            height={290}
                            style={{ height: "auto" }}
                        ...
                        <div className="content-wrapper">
                          <div className="content">
                            ...
                            <div className="meta">
                              <span className="name">
                                By <span>{typeof blog.author === "string" ? blog.author : "Admin"}</span>
                              </span>
