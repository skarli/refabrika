...
interface FeatureItem {
  _key?: string;
  imageSrc: typeof shape_1;
  title: string;
  text: string;
}

interface ServiceDetailsFeatureProps {
  service?: ServiceData;
}

// Fallback feature items with shapes
const fallbackFeatureItems: FeatureItem[] = [
  {
    imageSrc: shape_1,
...
const ServiceDetailsFeature = ({ service }: ServiceDetailsFeatureProps) => {
  const hasFeatureCards = service?.featureCards && service.featureCards.length > 0;

  const featureItems = hasFeatureCards
    ? service!.featureCards!.map((card, idx) => ({
        _key: card._key,
        imageSrc: shapes[idx % shapes.length],
        title: card.title.replace(" ", " <br> "),
          ...
          <div className="features-wrapper-box fade-anim">
            <div className="features-wrapper">
              {featureItems.map((item, index) => (
                <div key={item._key || index} className="feature-box">
                  <div className="thumb">
