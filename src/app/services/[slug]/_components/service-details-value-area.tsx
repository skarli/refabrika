...
interface ServiceDetailsValueAreaProps {
  service?: ServiceData;
}

// Fallback value items
const fallbackValueItems: ValueItem[] = [
  {
    number: "2750",
...
const ServiceDetailsValueArea = ({ service }: ServiceDetailsValueAreaProps) => {
  const sectionTitle = service?.valueSectionTitle ||
    "We sharpen your brands and businesses create exceptional experiences where people live work";

  const hasValueStats = service?.valueStats && service.valueStats.length > 0;
  const valueItems = hasValueStats
    ? service!.valueStats!.map((stat) => ({
        _key: stat._key,
        number: stat.value,
            ...
            <div className="section-content">
              ...
              <div className="values-wrapper">
                {valueItems.map((item, index) => (
                  <div key={item._key || index} className="value-box">
